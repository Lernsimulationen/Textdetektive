...
(() => {
  ...
  function render() {
    ...
    if (state && view === 'teacher') { const mode = document.getElementById('mode'), group = document.getElementById('group'); mode.value = state.mode; group.checked = state.group; mode.onchange = () => update({mode:mode.value}); group.onchange = () => update({group_mode:group.checked}); const qr = document.getElementById('qr'); if (qr) qr.innerHTML = qrSvg(`${location.href.split('#')[0]}#student?code=${state.code}`); }
  ...
  // QR Code, Version 6 / Fehlerkorrektur L. Reicht für die GitHub-Pages-Einladungs-URL.
